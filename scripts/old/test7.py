...
def process_file(file_path, output_path):
    # Try UTF-8 first, fallback to Latin-1 for European encodings
    try:
    ...
    return input_unique_ids, output_unique_ids
...
if __name__ == "__main__":
    ...
    for file_path in input_dir.glob('*.csv'):
        ...
        try:
            in_count, out_count = process_file(file_path, output_path)
            print(f"Processed: {file_path.name} | Unique IDs (Input: {in_count} -> Output: {out_count})")
