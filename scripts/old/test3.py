...
def process_file(file_path, output_path):
    ...
    # Process fuel names dynamically: remove spaces, convert to lowercase
    df['fuel_mapped'] = df['descCarburante'].astype(str).str.replace(' ', '').str.lower()
    
    # Process isSelf dynamically
    ...
    # Order columns: id_pump first, dynamic price columns middle, date last
    price_cols = [col for col in pivot_df.columns if col not in ('id_pump', 'date')]
    final_cols = ['id_pump'] + price_cols + ['date']
